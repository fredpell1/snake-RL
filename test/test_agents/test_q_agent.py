...
def value_function_vector():
    return torch.nn.Sequential(
        torch.nn.Linear(100, 50), torch.nn.ReLU(), torch.nn.Linear(50, 4)
    )
...
def agent_vector(value_function_vector):
    return DQNAgent(
        ...
        optimizer=torch.optim.AdamW(
            value_function_vector.parameters(), 0.0001, amsgrad=True
        ),
...
def agent_grid(value_function_cnn):
    return DQNAgent(
        ...
        epsilon_decay=2000,
        optimizer=torch.optim.AdamW(
            value_function_cnn.parameters(), 0.0001, amsgrad=True
        ),
        loss_function=torch.nn.HuberLoss(),
...
def agent_multiframe(value_function_multiframe):
    return DQNAgent(
        ...
        epsilon_decay=2000,
        optimizer=torch.optim.AdamW(
            value_function_multiframe.parameters(), 0.0001, amsgrad=True
        ),
        loss_function=torch.nn.HuberLoss(),
